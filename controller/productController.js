...
export const createProductController=async(req,res)=>{
    try{
     ...
     const { photo } = req.files;  // <-- FIXED: define photo properly
            ...
            case !name:
              return res.status(500).send({error:'Name is reqired'})
               ...
               case !description:
                ...
                return res.status(500).send({error:'photo is reqired and should be less then  mb'})
        ...
        res.status(201).send({
            success:true,
            message :"product created successfilly",
          product  ,
        })
    }catch(error){
        console.log(error)
